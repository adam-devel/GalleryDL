...
import it.matteoleggio.gallerydl.App
...
import it.matteoleggio.gallerydl.work.DownloadWorker
...
class DownloadViewModel(private val application: Application) : AndroidViewModel(application) {
    ...
    fun createDownloadItemFromResult(resultItem: ResultItem, type: Type) : DownloadItem {
        ...
        val customFileNameTemplate = when(type) {
            Type.image -> sharedPreferences.getString("file_name_template", "%(uploader)s - %(title)s")
            else -> ""
        }

        val downloadPath = when(type){
            Type.image -> sharedPreferences.getString("image_path", getApplication<App>().resources.getString(R.string.image_path))
    ...
    fun createDownloadItemFromHistory(historyItem: HistoryItem) : DownloadItem {
        ...
        val downloadPath = when(historyItem.type){
            Type.image -> sharedPreferences.getString("video_path", getApplication<App>().resources.getString(R.string.image_path))
            else -> sharedPreferences.getString("command_path", getApplication<App>().resources.getString(R.string.command_path))
        }
